...
Block: Bots, spammers, bad actors, careless, confused..''',
  ...
  static final Map<TrustVerb, List<_FieldDef>> _fieldConfigs = {
      ...
      _FieldDef(
        ...
        ({required key, initialValue, required onChanged, enabled = true, required context}) =>
            TextBoxEditor(
              ...
              initialValue: initialValue,
              hint: 'E.g. "Colleague", "College roommate", "Neighbor"',
              onChanged: onChanged,
            ),
      ),
    ],
    ...
    TrustVerb.delegate: [
      ...
      _FieldDef(
        ...
        ({required key, initialValue, required onChanged, enabled = true, required context}) =>
            DelegateRevokeAtEditor(key: key, initialRevokeAt: initialValue, onChanged: onChanged),
      ),
    ],
    TrustVerb.replace: [
      _FieldDef(
  ...
  String get _title {
    ...
    switch (_selectedVerb) {
      case TrustVerb.trust:
        if (widget.isNewScan) {
          return '''Who's Key is This?''';
        }
        return '$newOrUpdate Vouch';
      case TrustVerb.block:
        return '$newOrUpdate Block';
      case TrustVerb.replace:
        return '$newOrUpdate Key Replacement';
      case TrustVerb.delegate:
        return '$newOrUpdate Delegate';
      default:
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final bool isRevokeExposed =
          _fieldConfigs[_selectedVerb]?.any((f) => f.name == 'revokeAt') ?? false;
